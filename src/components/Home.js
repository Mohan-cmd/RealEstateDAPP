...
const Home = ({ home, account, provider, escrow, togglePop }) => {
   ...
   const fetchDetails = async () =>{
    //Buyer
     ...
     const hasInpected = await escrow.inspectionPassed(home.id)
     setHasInspected(hasInpected);
   ...
   const lendHandler = async ()=>{
    ...
    //Lender sends dunds to contract
               ...
               {owner ? (
                ...
                <ul>
                    {home.attributes.map((attribute,index)=>
                      <li key={index}><strong>{attribute.trait_type} : {attribute.value}</strong></li>
                    )}
                </ul>
